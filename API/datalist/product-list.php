...
<datalist id="productList">
    ...
    $query = "SELECT inventory_tb.InventoryID, itemtype_tb.itemTypeCode, inventory_tb.description,Status,  itemtype_tb.is_consumable FROM inventory_tb LEFT JOIN itemtype_tb ON inventory_tb.itemTypeID = itemtype_tb.itemTypeID";
    $productsResult = $conn->query($query);
    $productsData = (object)array(); // Create an empty object
    while ($row = $productsResult->fetch_assoc()) {
        $itemCode = $row['InventoryID'];
