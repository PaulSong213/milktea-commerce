...
<body>
    <main class="modal fade" data-bs-backdrop="static" id="printModal" tabindex="-1" aria-labelledby="printModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <div class="modal-header">
                    <div class="d-flex align-center">
                        <h5 class="modal-title" id="printModalLabel">Print Request Form</h5>
                    </div>

                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div id="xray-form-container" class="modal-body border p-4 m-4 shadow mx-auto" style="overflow-x: auto; width: max-content; max-width: 100%">
                    <div class="px-1" id="xray-form" style="width: 600px;">
                        <!-- HEADER -->
                        ...
                        <div class="d-flex flex-column mb-1" id="servicesAvailedContainer"></div>

                        <!-- FOR X-RAY ONLY FIELD -->
                        <div class="row ps-1 py-1 X-Ray-only d-none">
                            <div class="col-12 pe-0 d-flex">
                                <span>CHIEF COMPLAINT:</span>
                                <span id="formCheifComplaint" class="flex-grow-1 border-bottom border-1 border-dark d-block ps-1 fw-bold"></span>
                            </div>
                            <div class="col-12 pe-0 d-flex">
                                <span>WORKING DX:</span>
                                <span id="workingDXComplaint" class="flex-grow-1 border-bottom border-1 border-dark d-block ps-1 fw-bold"></span>
                            </div>
                        </div>
                ...
                <div class="modal-footer">
                    ...
                    <button id="print-xray-form" type="button" class="btn btn-primary">Print</button>
                </div>
            </div>
        </div>
    </main>
    ...
    <script>
        async function showXrayForm(serviceID, appendToElement = null) {
            try {
                ...
                if (appendToElement) {
                    ...
                    $(appendToElement).append($("#xray-form-container").html());
                } else {
                    var exampleModalPopup = new bootstrap.Modal($('#printModal'), {});
                    exampleModalPopup.show();
                }

                $("#print-xray-form, #print-xray-form-header").click(function() {
                    printXrayForm();
                });
                return service;
            } catch (error) {
        ...
        <?php
        if (isset($_SESSION['printXrayFormId'])) {
            $printXrayFormId = $_SESSION['printXrayFormId'];
            echo "showXrayForm(`" . $printXrayFormId . "`)";
            unset($_SESSION['printXrayFormId']);
        }
        ?>

        function printXrayForm() {
            var divToPrint = document.getElementById('xray-form').innerHTML;
